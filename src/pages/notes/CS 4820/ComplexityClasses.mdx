...
representing an input string and \\(t\\) a verification string used to check
if \\(s\\) is valid

this checker must run in polynomial time
...
**NP** is any problem with an efficient (polynomial time) certifier
...
<figure>
<figcaption>
Proving NP completeness for some problem \\(B\\)
</figcaption>

<p>
Given an NP-complete problem \\(A\\) show \\(A \leq_R B \\)
</p>
</figure>
...
- show problem is NP
- try to reduce arbitrary example of existing NP-complete problem into current problem
